...
// HACK: See https://github.com/jaredhanson/passport-twitter/issues/26
var InternalOAuthError = require('passport-twitter/node_modules/passport-oauth').InternalOAuthError;
PASSPORT_TWITTER.Strategy.prototype.userProfile = function(token, tokenSecret, params, done) {
			...
			fetchContacts: function(passportSession, service, options, callback) {
				...
				function checkForErrors(response, callback) {
					...
					if (typeof response === "string") {
						if (response.indexOf("Twitter is over capacity.") !== -1) {
							err = new Error("Over capacity");
		    	...
		    	function ensureUserInfo(callback) {
		    		...
		    		return requester(function(callback) {
			    		...
			    		initOptions.logger.info("[rolodex][twitter] Fetch user info for: " + passportSession.displayName);

			    		var url = "https://api.twitter.com/1.1/users/show.json?user_id=" + passportSession.id;
						...
						return REQUEST.get({
							url: url,
							oauth: {
								consumer_key: passportSession.consumerKey || config.passport.consumerKey,
						        consumer_secret: passportSession.consumerSecret || config.passport.consumerSecret,
		    	...
		    	return ensureUserInfo(function(err) {
		    		...
		    		initOptions.logger.info("[rolodex][twitter] Fetch contacts for: " + passportSession.displayName);
