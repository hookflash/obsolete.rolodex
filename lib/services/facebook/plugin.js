			...
			fetchContacts: function(passportSession, service, options, callback) {
		    	...
		    	function ensureUserInfo(callback) {
					...
					return requester(function(callback) {
						...
						return REQUEST.get({
//							url: "https://graph.facebook.com/v2.1/me?fields=id,name,picture&method=GET&format=json&access_token=" + passportSession.accessToken,
							url: "https://graph.facebook.com/v1.0/me?fields=id,name,picture&method=GET&format=json&access_token=" + passportSession.accessToken,
							json: true
...
/*
					function fetchFirendsLists(callback) {

						return requester(function(callback) {
							var pageUrl = "https://graph.facebook.com/v2.1/me/friendlists?limit=500&method=GET&format=json&access_token=" + passportSession.accessToken;
							initOptions.logger.debug("[rolodex][facebook] pageUrl:", pageUrl);
							return REQUEST.get({
								url: pageUrl,
								json: true
							}, function (err, res, friendlists) {
								if (err) return callback(err);
								var lists = {};
								if (friendlists.data) {
									friendlists.data.forEach(function (list) {
										lists[list.id] = list;
									});
								}
					...
					return fetchFirendsLists(function(err, friendlists) {
						if (err) return callback(err);
*/
...
/*
			    		...
			    		for (var id in friendlists) {
			    			waitfor(id, function (id, done) {

								initOptions.logger.debug("[rolodex][facebook] list ID:", id);

					    		return fetchPage("https://graph.facebook.com/v2.1/" + id + "/members?access_token=" + passportSession.accessToken, function(err) {
					    			if (err) return done(err);
*/
//					    		return fetchPage("https://graph.facebook.com/v2.1/me/taggable_friends?fields=id,name,picture&limit=500&method=GET&format=json&access_token=" + passportSession.accessToken, function(err) {
					    		return fetchPage("https://graph.facebook.com/v1.0/me/friends?fields=id,name,picture&limit=500&method=GET&format=json&access_token=" + passportSession.accessToken, function(err) {
					    			if (err) return callback(err);
