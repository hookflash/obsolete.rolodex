...
// TMP: Remove once new demo is live.
	...
	app.post(/^(?:\/\.openpeer-rolodex)?\/rolodex-access$/, function(req, res, _next) {
		try {
			...
			return self.getServicesSessionForToken(req.sessionID, request.$domain, request.rolodex.serverToken, function(err, servicesSession) {
...
Rolodex.prototype.getServicesSessionForToken = function(sessionID, domain, token, callback) {
	var self = this;
	...
	return getTokenSecret(false, function(err, tokenSecret) {
		if (err) return callback(err);
		return self.services.getForToken(sessionID, token, tokenSecret, function(err, servicesSession) {
			if (err) {
				if (err.code === "TOKEN_DECRYPT_ERROR") {
					...
					return getTokenSecret(true, function(err, tokenSecret) {
						...
						return self.services.getForToken(sessionID, token, tokenSecret, callback);
					});
				}
				return callback(err);
			}
