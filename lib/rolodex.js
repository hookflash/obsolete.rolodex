		...
		try {
			...
			function syncServiceStatusFromPassport(options, callback) {
				...
				return self.services.getForSessionID(req.sessionID, function(err, servicesSession) {
					...
					if (!req.session) return callback(new Error("No `req.session` available"));
					function syncService(serviceId) {
						if (
							...
							if (!options.serviceId || serviceId === options.serviceId) {
								// NOTE: We don't wait for this to complete.
								servicesSession.services[serviceId].error = false;
								self.provisionedServices[serviceId].fetchContacts(
									req.session.passport.user[serviceId],
									servicesSession.services[serviceId],
									options,
									function(err) {										
										if (err) {
											self.logger.warn("[rolodex] WARNING: Error fetching contacts", err.stack);
											servicesSession.services[serviceId].error = err.message;
										}
										return servicesSession.save(function(err) {
											if (err) {
												self.logger.warn("[rolodex] WARNING: Error saving services session", err.stack);
											}
										});
									}
								);
						...
						if (servicesSession.services[serviceId].loggedin) {
							servicesSession.services[serviceId].loggedin = false;
							servicesSession.save(function(err) {
								if (err) {
									self.logger.warn("[rolodex] WARNING: Error saving services session", err.stack);
								}
							});
			...
			if (name === "contacts") {
				return self.services.getForSessionID(req.sessionID, function(err, servicesSession) {
					...
					var serviceId = null;
					if (req.params[0]) {
						serviceId = serviceIdForParams(req.params);
					}
					return res.end(JSON.stringify(servicesSession.getContacts(serviceId)));
				});
			} else
			if (name === "refetch") {
				return syncServiceStatusFromPassport({
					refetch: true,
					serviceId: serviceIdForParams(req.params)
				}, function(err) {
					if (err) return next(err);
			...
			if (name === "logout") {
				...
				if (
					...
					delete req.session.passport.user[serviceId];
				}
				return syncServiceStatusFromPassport({
					refetch: false,
					serviceId: serviceId
			...
			if (name === "authCallback") {
				...
				return passport.authenticate(serviceId, function(err, user, info) {
					...
					return req.logIn(DEEPMERGE((req.session.passport && req.session.passport.user) || {}, user), function(err) {
				      if (err) return next(err);
