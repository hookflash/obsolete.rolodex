...
// TMP: Remove once new demo is live.
	...
	app.post(/^\/rolodex-access$/, function(req, res, next) {
		try {
			var request = req.body.request;
			var token = request.rolodex.serverToken;
			return self.getServicesSessionForToken(request.rolodex.serverToken, function(err, servicesSession) {
				if (err) return next(err);

				var fetching = servicesSession.getServices()[servicesSession._tokenData.service].fetching;
				...
				var payload = JSON.stringify({
				  "result": {
				    ...
				    "rolodex": {
				      ...
				      "updateNext": Math.round(Date.now()/1000) + ((fetching) ? 10 : -60*60)
				    }
	...
	app.post(/^\/rolodex-contacts-get$/, function(req, res, next) {
		...
		try {
			...
			if (request.rolodex.version) {
				res.writeHead(409);
				return res.end("Do not specify `rolodex.version`. Contact deltas not yet implemented!");
			}
			...
			return self.getServicesSessionForToken(request.rolodex.serverToken, function(err, servicesSession) {
				...
				function refresh(callback) {
					...
					if (servicesSession.services[servicesSession._tokenData.service].contactsTotal === 0) {
						servicesSession.services[servicesSession._tokenData.service].fetching = 0;
					}
				...
				return refresh(function(err) {
					...
					servicesSession.getContacts(servicesSession._tokenData.service, function(err, contacts) {
						...
						var payload = JSON.stringify({
						  "result": {
						    ...
						    "rolodex": {
						      ...
						      "updateNext": Math.round(Date.now()/1000) + ((fetching) ? 10 : 60*60*24),
						      // TODO: Implement versioning and only send deltas.
						      "version": ""
						    },
						    "identities": {
						    	"identity": identities
...
Rolodex.prototype.getServicesSessionForToken = function(token, callback) {
	return this.services.getForToken(token, callback);
}
