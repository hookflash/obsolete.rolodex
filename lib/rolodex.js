			...
			function syncServiceStatusFromPassport(options, callback) {
				...
				return self.services.getForSessionID(req.sessionID, function(err, servicesSession) {
					...
					for (var serviceId in self.provisionedServices) {
						// Copy info from `passport` to `rolodex` as passport will only keep info for latest strategy.
						if (
							...
							if (!req.session.rolodex.services) {
								req.session.rolodex.services = {};
							}
							req.session.rolodex.services[serviceId] = req.session.passport.user[serviceId];
						}
						if (
							req.session.rolodex &&
							req.session.rolodex.services &&
							req.session.rolodex.services[serviceId]
						) {
