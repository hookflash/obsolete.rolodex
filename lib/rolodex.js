		...
		try {
			...
			self.db.on("ready", function() {
				...
				KICKQ.config({
					...
					redisNamespace: self.config.db.redis.prefix + "kickq:" + self.config.db.redis.hostname + ":",
...
Rolodex.prototype.registerRoute = function(app, routeName) {
	var self = this;
	app[(routeName === "auth") ? "post" : "get"](self.routes[routeName], function(req, res, next) {
		try {
			if (!self.ready) {
