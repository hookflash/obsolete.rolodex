...
// TMP: Remove once new demo is live.
	...
	app.post(/^(?:\/\.openpeer-rolodex)?\/rolodex-access$/, function(req, res, next) {
		try {
			var request = req.body.request;
			var token = request.rolodex.serverToken;

			return self.getServicesSessionForToken(req.sessionID, request.rolodex.serverToken, function(err, servicesSession) {
				if (err) return next(err);

				var service = servicesSession.getServices()[servicesSession._tokenData.service];
console.log("ROLODEX ACCESS", service);
				var payload = null;
					...
					payload = {
					  "result": {
					    ...
					    "rolodex": {
					      ...
					      // If we are fetching on server we ask client to fetch contacts in 10 seconds otherwise immediately.
					      "updateNext": Math.round(Date.now()/1000) + ((service.fetching) ? 10 : -60*60)
					    }
	...
	// TODO: Decide on `/.openpeer-rolodex` or not but not optional.
	app.post(/^(?:\/\.openpeer-rolodex)?\/rolodex-contacts-get$/, function(req, res, next) {
...
console.log("ROLODEX GET-CONTACTS", service);
				...
				return servicesSession.getContacts(servicesSession._tokenData.service, function(err, contacts) {
					if (err) return next(err);

					function getDelta(callback) {
						if (!request.rolodex.version) {
						...
						return servicesSession.getCntactsForServiceAndTimestamp(servicesSession._tokenData.service, request.rolodex.version, function(err, result) {
							if (err) return callback(err);
							...
							for (var contactId in result.fetched) {
								if (!contacts[contactId]) {
									delta.push({
									    ...
									    // TODO: Don't assume `.com`.
							            "uri": "identity://" + servicesSession._tokenData.service + ".com/" + contactId,
									    // TODO: Get service plugin to format.
							            "provider": servicesSession._tokenData.service,
									});
								}
							}
					...
					function refresh(callback) {
						...
						return servicesSession.fetchForService(servicesSession._tokenData.service, callback);
					}
							...
							if (delta === false) {
								...
								for (var contactId in contacts) {
									identities.push({
							            "$disposition": "update",
									    // TODO: Get service plugin to format.
							            "uri": "identity://" + servicesSession._tokenData.service + "/" + contactId,
									    // TODO: Get service plugin to format.
							            "provider": servicesSession._tokenData.service,
							            "name": contacts[contactId].fn || contacts[contactId].nickname,
									    // TODO: Get service plugin to format.
								...
								return sendResponse({
								    "rolodex": {
								      ...
								      "updateNext": Math.round(Date.now()/1000) + ((service.fetching) ? 10 : 60*60*24),
...
Rolodex.prototype.getServicesSessionForToken = function(sessionID, token, callback) {
	return this.services.getForToken(sessionID, token, callback);
}
