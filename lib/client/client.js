...
define([
	...
	function Rolodex(options) {
		...
		if (self._options.loadContacts !== false) {
	...
	Rolodex.prototype.getFullContact = function(uid) {
		var self = this;
		// TODO: Cache this data until contact is updated (re-fetched)?
		var deferred = Q.defer();
		WINDOW.$.ajax({
			method: "POST",
			dataType: "json",
			url: self._options.baseURL + self._routes.contacts + "/" + uid.replace(":", "/"),
			xhrFields: {
                withCredentials: true
            },
            crossDomain: true
	...
	Rolodex.prototype.getContacts = function(serviceId, filter, forceFetch) {
		...
		if (self._options.loadContacts === false) {
			console.log("Contact loading disabled.");
			return Q.resolve({});
		}
					...
					for (var property in filter) {
						...
						if (contacts[contactId][property]) {
							if (typeof filter[property] === "string") {
								if (filter[property] === contacts[contactId][property]) {
									matched = true;
								}
							} else
		...
		if (serviceId) {
			...
			return self.getServices().then(function(services) {
				...
				function forService(serviceId) {
						...
						return getContactsFor(serviceId, forceFetch).then(function(serviceContacts) {
	...
	Rolodex.prototype.loginService = function(serviceId, forceFreshLogin) {
		var self = this;
		return self.getServices().then(function(services) {
			var form = WINDOW.$("<form/>").attr({
			...
			if (forceFreshLogin) {
				form = form.append(WINDOW.$("<input/>").attr({
					"type": "hidden",
					"name": "fresh",
					"value": "true"
				}));
			}
			WINDOW.$("body").append(form).find("#rolodex-auth-form").submit();
		});
	}
