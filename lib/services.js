...
Services.prototype.getForSessionID = function(sessionID, callback) {
	...
	// Load existing session data from db if available.
...
ServicesSession.prototype.save = function(callback) {
return callback(null);
	var self = this;
	// TODO: Have records expire after some time for stale users (e.g. 1 month).
    return self.rolodex.db.set(self._getDBKey(), JSON.stringify(self.services), function (err, reply) {
    	if (err) return callback(err);
		return callback(null);
    });
}
