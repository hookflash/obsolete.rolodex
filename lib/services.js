...
Services.prototype.getForToken = function(token, callback) {
	var self = this;
	function decryptToken(callback) {
		...
		function decryptWithString(key, iv, data) {
		...
		var tokenData = decryptWithString(calcSHA256(self.rolodex.config.api.tokenSecret), UTILS.fromhex(tokenParts[0]), tokenParts[1]);
	...
	return decryptToken(function(err, credentials) {
		if (err) return callback(err);

		try {

			var id = UTILS.tohex(MD5(token.split("-")[0] + ":" + credentials.token));
...
ServicesSession.prototype.loginService = function(serviceId, credentials, callback) {
	...
	function setToken(callback) {
		...
		*/
		...
		function encryptWithString(key, iv, data) {
			var length = data.length;
			var state = new Array(length);
			for (var i = 0; i < length; i++) {
				state[i] = data.charCodeAt(i);
		...
		return CRYPTO.randomBytes(16, function(err, buffer) {
			...
			var iv = MD5(buffer.toString("hex"));
			service.token = UTILS.tohex(iv) + "-" + encryptWithString(calcSHA256(self.rolodex.config.api.tokenSecret), iv, JSON.stringify(tokenInfo));

//console.log("self.rolodex.config.api.tokenSecret", self.rolodex.config.api.tokenSecret);
