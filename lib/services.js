...
function encryptToken(tokenInfo, tokenSecret, callback) {
	return CRYPTO.randomBytes(32, function(err, buffer) {
		...
		return callback(null, iv + "-" + encrypt(sha256(tokenSecret), iv, JSON.stringify(tokenInfo)));
	});
}
...
Services.prototype.getForToken = function(sessionID, token, callback) {
	var self = this;

	return decryptToken(token, self.rolodex.config.api.tokenSecret, function(err, credentials) {
		if (err) return callback(err);

		try {

			var id = sessionID || UTILS.tohex(MD5(token.split("-")[0] + ":" + credentials.token));
...
ServicesSession.prototype.fetchForService = function(serviceId, callback) {
	...
	function scheduleFetch(callback) {
		if (service.lastFetchTime && service.lastFetchTime > (Math.floor(Date.now()/1000) - 60) && self.options.test !== true) {
			return callback(null, {
				error: {
