...
Services.prototype.getForToken = function(token, callback) {
	...
	function decryptToken(callback) {
		...
		var tokenData = decryptWithString(self.rolodex.config.api.tokenSecret, UTILS.fromhex(tokenParts[0]), tokenParts[1]);
...
ServicesSession.prototype.loginService = function(serviceId, credentials, callback) {
	...
	function setToken(callback) {
		/*
		<sharedSecret> = hash (32 char)

		<iv> = MD5 random hash (32 char)
		token = <iv> + "-" + base64encode(AES.encrypt(<sharedSecret>, <iv>, <credentials>))
		...
		*/
		...
		return CRYPTO.randomBytes(16, function(err, buffer) {
			...
			service.token = UTILS.tohex(iv) + "-" + encryptWithString(self.rolodex.config.api.tokenSecret, iv, JSON.stringify(tokenInfo));
